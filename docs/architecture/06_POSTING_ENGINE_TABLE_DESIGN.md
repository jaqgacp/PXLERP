...
The posting engine converts source documents (invoices, receipts, vouchers) into balanced journal entries following pre-defined posting rules. This document covers all tables that power the posting engine.
...
| `rule_set_code` | text | UNIQUE per company, NOT NULL | e.g., 'SALES_INV_VAT', 'PV_EWT' |
| `transaction_type` | text | NOT NULL | 'sales_invoice' \| 'vendor_bill' \| 'receipt' \| 'payment_voucher' \| 'petty_cash_voucher' \| 'inventory_adjustment' \| 'asset_depreciation' \| 'bank_deposit' \| 'bank_withdrawal' \| 'bank_transfer' \| 'journal_entry' |
| `description` | text | | |
...
| `account_config_key` | text | NULL | Used if account_source='FROM_SYSTEM_CONFIG'; e.g., 'AR_TRADE', 'OUTPUT_VAT', 'AP_TRADE' |
...
| `subsidiary_ledger_type` | text | NULL | 'AR' \| 'AP' \| 'INVENTORY' \| 'FIXED_ASSET' \| NULL |
...
| `description_template` | text | NULL | e.g., 'Sales Invoice {doc_number} - {customer_name}' |
...
| `id` | uuid | PK | |
| `company_id` | uuid | FK companies, NOT NULL | |
| `config_key` | text | NOT NULL | e.g., 'AR_TRADE', 'AP_TRADE', 'OUTPUT_VAT', 'INPUT_VAT', 'EWT_PAYABLE', 'CASH_ON_HAND', 'RETAINED_EARNINGS' |
...
| `document_number` | text | NOT NULL | System-assigned JE number |
...
CONSTRAINT: Cannot update any column if `status = 'POSTED'` — enforced by trigger.
...
Detailed ledger per customer (AR), supplier (AP), item (inventory), asset.
...
| `unlock_reason` | text | NULL | If ever unlocked |
...
| `is_active` | boolean | NOT NULL DEFAULT true | |
| `auto_post` | boolean | NOT NULL DEFAULT false | If true, post immediately |
| `created_at` | timestamptz | NOT NULL DEFAULT now() | |
| `created_by` | uuid | FK auth.users | |
...
|---|---|---|---|
| `id` | uuid | PK | |
| `template_id` | uuid | FK recurring_journal_templates | |
| `line_number` | integer | NOT NULL | |
| `account_id` | uuid | FK chart_of_accounts | |
| `branch_id` | uuid | FK branches, NULL | |
| `department_id` | uuid | FK departments, NULL | |
| `cost_center_id` | uuid | FK cost_centers, NULL | |
| `entry_side` | text | CHECK IN ('DEBIT','CREDIT') | |
...
```
1. VALIDATE source document (all required fields present)
2. CHECK fiscal period is OPEN (fiscal_locks)
...
8. UPSERT gl_balances (per account per period)
9. INSERT subsidiary_ledger_entries (AR / AP / inventory / FA)
10. UPDATE source document status → 'POSTED'
11. INSERT document_relationships (source → JE)
12. INSERT audit_logs (event_type='DOCUMENT_POSTED')
```

All steps execute within a single database transaction. Failure at any step rolls back entirely.
