...
|---|---|---|
| OD-V3-T1 | `income_tax_computation_lines` — on-demand or continuous? | **RESOLVED v3.7:** On-demand per ITR run (idempotent). `income_tax_computation_lines` are generated by the `itr_computation_runs` engine, not accumulated continuously. On each run, all lines for that `computation_run_id` are DELETED then re-INSERTed (fully idempotent). This allows corrections and what-if scenarios before marking `is_final=true`. Developer: DELETE WHERE `computation_run_id=?` then INSERT fresh set. |
| OD-V3-T2 | `itr_computation_runs.is_final = true` — lock or allow recomputation? | **RESOLVED v3.7:** Multiple runs allowed; `is_final=true` marks the filing run but does NOT hard-lock. Accountant may un-mark `is_final` and re-run with a new run record if an amendment is needed. Only one run per `(company_id, fiscal_year_id)` may have `is_final=true` at a time — enforce via partial unique index `WHERE is_final=true`. |
| OD-V3-T3 | `book_tax_reconciliations` — line-by-line or summary? | **RESOLVED v3.7:** Line-by-line. Each `book_tax_reconciliation_line` represents one reconciling item (e.g., non-deductible entertainment expense, NOLCO carry-forward, tax-exempt income). Summary total is derived from lines. This matches BIR Form 1702-MX Schedule 2 (Reconciliation of Net Income per Books vs Taxable Income) which requires line disclosure. |
| OD-V3-T4 | `tax_credits_schedules` — link to `certificates_2307_received` 1:many or summary? | **RESOLVED v3.7:** 1:many link to `certificates_2307_received`. Each `tax_credits_schedules` line links to one or more `certificates_2307_received` records via FK `certificate_2307_id`. Summary totals are derived. This enables traceability — each tax credit must be supported by a specific 2307 certificate. Developer: `tax_credits_schedules.certificate_2307_id uuid NULL FK → certificates_2307_received.id`. |
...
| # | Question | **RESOLUTION** |
|---|---|---|
| OD-09 | EWT on petty cash — captured at voucher or at replenishment? | **RESOLVED v3.7:** Captured at petty_cash_voucher line level. `petty_cash_voucher_lines` includes `ewt_atc_id` and `ewt_amount`. The posting engine writes `ewt_entries` at voucher posting. The replenishment payment voucher does NOT re-capture EWT — EWT is already booked. Developer: treat petty_cash_voucher the same as cash_purchase for EWT write. |
| OD-10 | `bank_statement_lines` — CSV import only or bank API in Phase 1? | **RESOLVED v3.7:** CSV import only in Phase 1. Bank API integration is Phase 2. `bank_statement_lines` are created via import_batches (import_type='bank_statement'). No real-time bank connection in Phase 1. Developer: bank_statement_lines.import_batch_id FK → import_batches.id is sufficient for Phase 1. |
| OD-11 | `budget_lines` — department level or account level only? | **RESOLVED v3.7:** Account + department level. `budget_lines` stores one row per (budget_id, account_id, department_id, fiscal_period_id). `department_id` is nullable — NULL = company-wide budget for that account. This supports both account-only budgets (department_id=NULL) and department-level budgets (department_id=specific). No branch-level budget in Phase 1 (branch_id is also nullable; NULL = all branches). |
| OD-12 | `notifications` — in-app only or include email? | **RESOLVED v3.7:** In-app only for Phase 1. `notification_delivery_logs` will only create records with `channel='in_app'`. Email delivery is Phase 2 — the `notification_templates` and `notifications` tables are designed for multi-channel but Phase 1 only populates `channel='in_app'` delivery. Developer: do NOT wire email SMTP in Phase 1; focus on `notifications` INSERT + real-time subscription. |
